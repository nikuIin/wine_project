...
const AnimatedItem: React.FC<AnimatedItemProps> = ({
  ...
  return (
    <motion.div
      ...
      className={`mb-4 cursor-pointer hover:rotate-1 duration-150 ${isSelected ? "rotate-1" : ""}`}
    >
      {children}
    </motion.div>
  );
    ...
    <div className={`relative ${className}`}>
      ...
      >
        {items.map((item, index) => (
          ...
          >
            <div
              className={`p-4 bg-white dark:bg-black dark:border-white border-black border-1 rounded-lg ${selectedIndex === index ? "bg-[#222]" : ""} ${itemClassName}`}
            >
